...
/**
 对于自定义Segment设置选中的viewcontroller
 */
- (void)customSegmentSelectedIndex:(NSInteger)selectedIndex {
    _selectIndex = (int)selectedIndex;
    if (!_hasInited) return;
    int currentIndex = _selectIndex;
    _startDragging = NO;
    CGPoint targetP = CGPointMake(_contentViewFrame.size.width * selectedIndex, 0);
    ...
    [self wm_postFullyDisplayedNotificationWithCurrentIndex:(int)selectedIndex];
    [self didEnterController:self.currentViewController atIndex:selectedIndex];
}
...
- (void)wm_calculateSize {
    ...
    } else if (self.menuViewStyle == WMMenuViewStyleNone) {
...
- (void)wm_layoutChildViewControllers {
    int currentPage = (int)(self.scrollView.contentOffset.x / _contentViewFrame.size.width);//原始设计，开启滚动动画后，中间每个控制器都会滚过
//    int currentPage = (int)(self.selectIndex);
    int length = (int)self.preloadPolicy;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (self.bWMPageVCDidScroll && _startDragging) {
        self.bWMPageVCDidScroll(scrollView.contentOffset);
    }
