...
static char const * const kTZPopGestureKey = "kTZPopGestureKey";
static char const * const kTZNaviDelegateKey = "kTZNaviDelegateKey";

//+ (void)load {
//    static dispatch_once_t onceToken;
//    dispatch_once(&onceToken, ^{
////        [self le_swizzleSelector:@selector(viewDidLoad) toSelector:@selector(tzPop_viewDidLoad)];
//        [self le_swizzleSelector:@selector(viewWillAppear:) toSelector:@selector(tzPop_viewWillAppear:)];
//    });
//}

+ (void)le_swizzleSelector:(SEL)originalSelector toSelector:(SEL)swizzledSelector {
...
- (void)tzPop_viewDidLoad {
    [self tzPop_viewDidLoad];
...
- (id)tz_popDelegate {
    id tz_popDelegate = objc_getAssociatedObject(self, kTZPopGestureKey);
    if (!tz_popDelegate) {
        tz_popDelegate = self.interactivePopGestureRecognizer.delegate;
        objc_setAssociatedObject(self, kTZPopGestureKey, tz_popDelegate, OBJC_ASSOCIATION_ASSIGN);
    }
    return tz_popDelegate;
}

- (id)tz_naviDelegate {
    id tz_naviDelegate = objc_getAssociatedObject(self, kTZNaviDelegateKey);
    if (!tz_naviDelegate) {
        tz_naviDelegate = self.delegate;
        if (tz_naviDelegate) {
            objc_setAssociatedObject(self, kTZNaviDelegateKey, tz_naviDelegate, OBJC_ASSOCIATION_ASSIGN);
        }
    }
